    ...
    if last_step:
        ...
        save_checkpoint(
            ...
            optimizer.state_dict(),
            {
                ...
                # Save the FULL model config (asdict), not a hand-picked subset. The old subset
                # dropped non-default architecture flags (use_mla, n_mtp, use_gated_attn,
                # tie_embeddings, rope_*), so reloading an MLA/MTP model for eval/RL rebuilt a
                # vanilla GPT and crashed on a state_dict mismatch. Mirrors base_train.py.
